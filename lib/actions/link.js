...
const getFmt = require("../util").getFmt;
...
module.exports = (cfg, args) => {
  ...
  return Promise.all(cfg.map((obj) =>
    ...
    // Link (delete).
    .then((files) => {
      const filesList = files.length ? [""].concat(files).join("\n- ") : "";
      process.stdout.write(fmt({
        color: "gray", key: "link",
        msg: `Found ${files.length} directories to delete: ${filesList}`
      }));
