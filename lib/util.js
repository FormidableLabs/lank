...
const util = module.exports = {
  ...
  requireOrNull: (mod) => {
    try {
      ...
      // Disallow certain types of require errors.
  ...
  getFmt: (cfg) => {
    if (!util._fmt) {
      ...
      const longestProj = ((cfg || {}).projs || [])
        .reduce((acc, obj) => acc >= obj.module.length ? acc : obj.module.length, 0);
  ...
  exec: (cmd, shellOpts, opts) => new Promise((resolve, reject) => {
    ...
    // Either use real buffer or "unbuffered" shim.
    const buffer = opts.unbuffered
      ? { push: writeMsg,
        forEach: noop }
      : [];
