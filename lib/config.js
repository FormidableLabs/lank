...
const config = module.exports = {
  ...
  getConfig: async () => {
    ...
    const projPaths = [
      ...
      path.join(util._cwd(), LANKRC),
      // Then try, `${PWD}/(..|../..)/.lankrc.js`
      path.join(util._cwd(), siblingPath, LANKRC)
    ];
    let projs = projPaths.reduce((memo, p) => memo || requireOrNull(p), null);
    if (!projs) {
      return Promise.reject(
        new Error(`Unable to find configuration data. Tried: ${projPaths.join(", ")}`));
    }
